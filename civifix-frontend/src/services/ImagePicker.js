...
export async function launchImageLibraryAsync(options) {
  ...
  if(res.assets && res.assets.length > 0) return {canceled: false, assets: res.assets.map(a => ({uri: a.uri, base64: a.base64}))};
...
export async function launchCameraAsync(options) {
  ...
  if(res.didCancel) return {canceled: true};
  if(res.assets && res.assets.length > 0) return {canceled: false, assets: [{uri: res.assets[0].uri, base64: res.assets[0].base64}]};
  return {canceled: true};
}
